...
public class ResearchModuleMine extends Module implements Component {
    private final static int h = 1;
    private final static int w = 1;
    private final static double MAX_ANGLE = 10;

    public ResearchModuleMine(Module module) {
        super(module.getId(), module.getId_user(), module.getId_zone(),
                module.getModule_type(), module.getX(), module.getY());
    }

    public ResearchModuleMine(Long id, Long id_user, Integer id_zone, Integer module_type, Integer x, Integer y) {
    ...
    public Integer getRationality(List<Module> modules, List<Link> links, List<Resource> resources) {
        ...
        for(Module module : modules){
            ...
            if(module.getModule_type() == TypeModule.MINE_BASE.ordinal()) mine = true;
    ...
    public boolean cross(int x, int y, int w, int h) {
        return (x >= getX() && x <= getX() + ResearchModuleMine.w && y >= getY() && y <= getY() + ResearchModuleMine.h) ||
