...
public class WarehouseGases extends Module implements Component {
    private final static int h = 2;
    private final static int w = 2;
    private final static double MAX_ANGLE = 10;

    public WarehouseGases(Module module) {
        super(module.getId(), module.getId_user(), module.getId_zone(),
                module.getModule_type(), module.getX(), module.getY());
    }

    public WarehouseGases(Long id, Long id_user, Integer id_zone, Integer module_type, Integer x, Integer y) {
    ...
    public Integer getRationality(List<Module> modules, List<Link> links, List<Resource> resources) {
        ...
        long cur = resources.get(TypeResources.CO2.ordinal()).getCount() +
                resources.get(TypeResources.O2.ordinal()).getCount();
