...
public class CommunicationTower extends Module implements Component {
    ...
    public CommunicationTower(Module module) {
        super(module.getId(), module.getId_user(), module.getId_zone(),
                module.getModule_type(), module.getX(), module.getY());
    }

    public CommunicationTower(Long id, Long id_user, Integer id_zone, Integer module_type, Integer x, Integer y) {
    ...
    public boolean cross(int x, int y, int w, int h) {
        return (x >= getX() && x <= getX() + CommunicationTower.w && y >= getY() && y <= getY() + CommunicationTower.h) ||
