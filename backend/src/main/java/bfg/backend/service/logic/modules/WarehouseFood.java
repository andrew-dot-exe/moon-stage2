...
public class WarehouseFood extends Module implements Component {
    ...
    public Integer getRationality(List<Module> modules, List<Link> links, List<Resource> resources) {
        ...
        long cur = resources.get(TypeResources.FOOD.ordinal()).getCount() +
                resources.get(TypeResources.H2O.ordinal()).getCount();
    ...
    public boolean cross(int x, int y, int w, int h) {
        return (x >= getX() && x <= getX() + WarehouseFood.w && y >= getY() && y <= getY() + WarehouseFood.h) ||
